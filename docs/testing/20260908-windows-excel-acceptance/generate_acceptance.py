...
def write_workbook(path: Path, *, use_zip64: bool) -> None:
    with nx.Workbook(
        ...
        workbook.write_sheet(
            pl.DataFrame({"n": [1234.56], "edit_instruction": ["In desktop Excel, replace A2 with 1234567890123.45 and inspect the column."]}),
            "BestFit edit", column_formats={"n": nx.Format(num_format="0.00")},
...
(OUT / "generation.json").write_text(json.dumps({
    "source_id": "0.2.1-release-candidate",
    "package_version": __import__("importlib.metadata", fromlist=["version"]).version("neatxlsx"),
    "native_sha256": sha256(Path(__import__("neatxlsx._native", fromlist=["__file__"]).__file__)),
    "wheel_sha256": sha256(Path('/home/fqzhang/project/neatxlsx/.worktrees/single-pass-autofit/dist/neatxlsx-0.2.1-cp311-abi3-manylinux_2_28_x86_64.whl')),
